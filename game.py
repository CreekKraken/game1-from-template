...
def remove_spaces(text):
    ...
    ''
    """
    if text=="" or text=="   ":     #hard coded cheese right here
        return ""
    while text[0]==" " and text!="":
        text=text[1:]
...
def print_menu(exits):
    ...
    print("You can:")
    for e in exits:
        print_menu_line(e,rooms[exits[e]]["name"])
...
def menu(exits):
    ...
    while True:
        ...
        print_menu(exits)
        answer=input(">> ")
        ans=normalise_input(answer)
        if is_valid_exit(exits,ans):
            break
    return ans
